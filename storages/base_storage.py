...
class BaseStorage(ABC):
    ...
    def _create_storages_folder(self):
        self._get_storage_folder().mkdir(parents=True, exist_ok=True)

    def _get_storage_file_name(self) -> Path:
        return self._get_storage_folder() / self.STORAGE_FILE_NAME

    @abstractmethod
    def store_element(self, element: Hashable):
        pass

    def exist_element(self, element: Hashable) -> bool:
        data = self.get_data()
        return element in data if data else False

    def save_data(self, data: Dict):
        if not self._get_storage_file_name().exists():
